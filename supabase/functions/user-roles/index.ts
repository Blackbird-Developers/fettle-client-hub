    ...
    try {
      ...
      switch (req.method) {
        case "GET": {
          ...
          // Query profiles by user_id (which matches auth.users.id and user_roles.user_id)
          ...
          // Combine the data - map by user_id
        ...
        case "POST": {
          ...
          // Find user by email - get user_id which references auth.users
          ...
          // Insert the role - use profile.user_id (auth.users.id), not profile.id
          ...
          console.log(`Admin ${user.id} added ${role} role to user ${profile.user_id}`);

          // Return with profile info (we already have it from the query above)
          const data = {
            ...newRole,
            profiles: {
              id: profile.id,
